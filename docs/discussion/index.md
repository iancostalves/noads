...
Overall results show that, under trend demand growth:
- **Baseline scenarios** display a peak in emissions between 2035 and 2040
- **Drop-in mitigation** (SAF only) has limited emissions reductions due to energy availability constraints (2070 emissions still 75% of 2019 with preferential availability)
- **Breakthrough aircraft technologies** can allow reaching near zero emissions, but impact is delayed to after 2045 due to late Entry-Into-Service and slow fleet renewal

### Technology and System Dependencies

The choice of aircraft architecture and energy carrier is highly dependent on:
- **Vehicle performance**: Determined by TLAR and technology assumptions
- **Background energy system**: Timing of electricity decarbonization and limited availability of electricity and biomass

### Paris Agreement Alignment

To respect Paris Agreement targets under an effort-sharing principle:
- **Drop-in mitigation alone** puts system under higher stress: either constraining traffic or consuming too much biomass and electricity
- **Alternative aircraft** reduce such stress by making better use of the same energy resources, even if their energy consumption is higher than conventional planes

### Complementarity of Measures

When comparing mitigation policies with different objective functions:
- Supply caps, energy availability, and alternative aircraft designs are **complementary rather than competing measures**
- One strategy alone achieves emission reduction up to a certain level
- Their **combined use enables more efficient mitigation** by:
  - Using alternative aircraft in feasible markets (reducing needs for low-carbon electricity and biomass)
  - Avoiding emission-intense markets (further reducing fossil kerosene consumption)

### Methodological Contributions

Regarding numerical methods:
- **GEMSEO-JAX** reduced both implementation burden and execution time
- Simulation of optimization-based policy scenarios was **prohibitive without the speedups obtained**:
  - Two orders of magnitude speedup at scenario optimization level
  - Three orders of magnitude speedup at computation and linearization level

### Practical Insights

This research offers practical insights on:
- How to efficiently use optimization algorithms for mitigation scenarios
- How to optimally allocate aircraft architectures, energy resources, and demand-side measures
- Achieving stringent mitigation targets in the aviation sector

## Code Availability

All scripts and data required to reproduce results are openly available in the [GitHub repository](https://github.com/iancostalves/noads).
