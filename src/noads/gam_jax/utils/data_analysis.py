...
markers = {"piston": "x", "turboprop": "^", "turbofan": "o"}
...
def draw_grid(file_name, data_grid, title):
    ...
    range_list = np.array(data_grid[2]) * 1.0e-3
    ...
    plt.grid(True)
    fig.colorbar(im, ax=ax, orientation="vertical", aspect=40.0)
    plt.tight_layout()
...
def read_db(file):
    ...
    df = raw_data.iloc[2:, 0:].reset_index(
        drop=True
    )  # Remove unit rows and reset index
    ...
    for name in df.columns:
        ...
        if un.loc[1, name] == "mach":
            for j in df.index:
                if df.loc[j, name] > 1.0:
                    df.loc[j, name] = float(
                        unit.convert_from(un.loc[0, name], df.loc[j, name])
                    )
    return df, un
...
def lin_lst_reg(df, abs, ord, order):
    ...
    def make_mat(param, order):
        mat_list = [param**j for j in order]
    ...
    (C, res, _rnk, _s) = np.linalg.lstsq(A, B, rcond=None)
...
def plot_without_reg(
    ...
    if (markers is not None) and (colors is not None):
        msg = "Only markers or colors should be given"
        raise ValueError(msg)
    if colors is not None:
        draw_colored_cloud_on_axis(
            axes, df, un, abs, ord, colors, xrange=xrange, yrange=yrange
        )
    elif markers is not None:
        clouds = draw_markers_and_colormap_cloud_on_axis(
            axes,
            df,
            un,
            abs,
        ...
        fig.colorbar(
            ...
            label=color_by + "(" + un.loc[0, color_by] + ")",
        )
    else:
        msg = "You must specify colors or markers."
        raise ValueError(msg)
...
def draw_reg(
    df, un, abs, ord, reg, colors, title=None, leg_loc="lower right", file_name=None
):
    ...
    if len(reg[0]) > 0:
        plt.plot(
            unit.convert_to(un.loc[0, abs], reg[0]),
            unit.convert_to(un.loc[0, ord], reg[1]),
            linewidth=2,
            color="grey",
        )
...
def subplots_by_varname(df, un, var_names, figwidth=12, savefig=False):
    ...
    if len(var_names) < 1:  # Check the length of var_names
        msg = "var_names is empty"
        raise ValueError(msg)

    figsize = (figwidth, figwidth * 0.4 * len(var_names))
    _fig, axes = plt.subplots(
        len(var_names), 2, figsize=figsize
    )  # Create subplots and eventually refactor the axis list
    if len(var_names) == 1:
        axes = [axes]
    ...
    for line, var in zip(axes, var_names):  # fill the subplots
        ...
        draw_colored_cloud_on_axis(
            line[0], df, un, var, "model_" + var, xrange=var_range
        )  # draw x_model versus x_data
        ...
        draw_colored_cloud_on_axis(
            line[1], df, un, var, "error_" + var, xrange=var_range, yrange=[-100, 100]
        )
...
def draw_colored_cloud_on_axis(
    ax, df, un, abs, ord, colors, xrange=None, yrange=None, leg_loc="lower right"
):
    ...
    cloud = []
    for typ in colors:
        abs_list = unit.convert_to(
            un.loc[0, abs], list(df.loc[df["airplane_type"] == typ][abs])
        )
        ord_list = unit.convert_to(
            un.loc[0, ord], list(df.loc[df["airplane_type"] == typ][ord])
        )
        if len(abs_list) > 0:
            subcloud = ax.scatter(
                abs_list, ord_list, marker="o", c=colors[typ], s=10, label=typ
            )
...
def draw_markers_and_colormap_cloud_on_axis(
    ax,
    df,
    un,
    abs,
    ...
    for typ in markers:
        abs_list = unit.convert_to(
            un.loc[0, abs], list(df.loc[df["engine_type"] == typ][abs])
        )
        ord_list = unit.convert_to(
            un.loc[0, ord], list(df.loc[df["engine_type"] == typ][ord])
        )
        color_list = unit.convert_to(
            un.loc[0, color_by], list(df.loc[df["engine_type"] == typ][color_by])
        )
        if len(abs_list) > 0:
            subcloud = ax.scatter(
                abs_list,
...
if __name__ == "__main__":
    ...
    df1[abs] = (
        df1["n_pax"] * df1["nominal_range"] ** 0.75
    ...
    df1[abs] = (
        df1["n_pax"] * df1["nominal_range"] ** 0.25
    )  # Add the new column to the dataframe
    un1[abs] = "km"  # Add its unit
    ...
    df[abs] = (
        df["mtow"] / df["wing_area"]
    ) ** 0.5  # Add the new column to the dataframe
    ...
    df1[ord] = (
        df1["n_pax"] * df1["nominal_range"] / df1["mtow"]
    )  # Add the new column to the dataframe
