...
design_mission = {
    "category": "short_medium",  # Airplane category (determines, among others, the amount of furnishing)
...
}  # cruise altitude for the design mission (typically 35000 ft for jet aircraft and less than 20000 ft for turboprop)
...
tow = (
    my_airplane["mtow"] * 0.9
)  # (kg) let us assume the airplane takes-off at 90% of its maximum mass
distance = 2000e3  # (m) flight distance
cruise_speed = my_airplane[
    "cruise_speed"
]  # (-) cruise Mach number (or True Airspeed (m/s) )
mtow = my_airplane["mtow"]  # (kg) Maximum Take-Off Weight
total_power = my_airplane["total_power"]  # (W) Total propulsive power
altitude_data = my_airplane[
    "altitude_data"
]  # Cruise altitude, diversion altitude and holding altitude
reserve_data = my_airplane["reserve_data"]  # Typical fuel reserves data

total_fuel_dict = gam.total_fuel(
    tow,
    distance,
    cruise_speed,
    mtow,
...
for _key, _val in total_fuel_dict.items():
    pass
