...
class TemporalScenario(Model):
    ...
    The scenario chains the disciplines of the given models into a single
    differentiable JAX program evaluated over a yearly time vector. It manages five
    kinds of inputs, identified by name prefixes on the assembled discipline:

    - ``constant.<name>``: time-independent scalars broadcast to all time steps;
    - ``interpolate.<name>``: values given at the coarse interpolation knots and
      interpolated (linearly or cubically) onto the time vector;
    - ``control.<name>``: interpolation knots of a time-dependent control, applied
      through a first-order delay ODE (integrated with diffrax) whose time constant
      is set by ``control_delay_times``;
    - custom controls (e.g. the ramped-pulse aircraft market shares), whose delayed
      response is also integrated but whose input signal is built by the models;
    - remaining inputs, evaluated as-is at every time step.

    Outputs listed in ``time_integrated_outputs`` are also integrated over time and
    exposed as ``cumulative.<name>``; path-wise constraints are exposed as
    ``<name>.constraint`` (or time-integrated violations); and outputs listed in
    ``final_rates`` provide their rate of change at final time as
    ``final_rate.<name>``. The whole program is vectorized over time with
    ``jax.vmap``, JIT-compiled on demand, and differentiated in forward or reverse
    mode, with a configurable adjoint for the delay ODEs (see
    :class:`.AdjointMethod`).
    """
    ...
    def __init__(
        ...
        """Assemble the models into the time-dependent discipline.

        Args:
                models: The models to assemble; their couplings are resolved
                    automatically from input/output names.
                constant_inputs: The names of time-independent inputs.
                control_delay_times: The delay time constant (years) of each
                    time-dependent control, keyed by control name.
                constrained_control_groups: Groups of control names whose sum must stay
                    within [0, 1] at every time step (e.g. pathway shares of one energy,
                    aircraft shares of one market), keyed by group name.
                custom_controls: The controls whose input signal is generated by a model
                    (e.g. ramped pulses) instead of interpolation knots.
                final_rates: The outputs whose rate of change at final time is exposed
                    (used e.g. to ensure fleet turnover is complete by the end year).
                interpolated_inputs: The names of the inputs interpolated from the
                    coarse knots onto the simulation time vector.
                time_integrated_outputs: The outputs to integrate over time, exposed
                ...
                interp_step: The spacing in years of the control/interpolation knots.
                cubic_interpolation: Whether to use cubic instead of linear
                    interpolation.
                differentiation_method: The JAX automatic differentiation mode; forward
                    mode requires the direct adjoint.
                adjoint_method: The diffrax adjoint used to differentiate through the
                    delay ODE integration.

        Raises:
                ValueError: If the differentiation method is ``AUTO``, or if forward
                    differentiation is combined with an incompatible adjoint.
