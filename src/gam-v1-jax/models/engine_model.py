...
def psfc_thierry(pw):
    ...
    psfc = unit.convert_from("kg/kW/h", c1 + k1 / unit.kW_W(pw)**p1)
...
def tsfc_thierry(bpr, vtas):
    ...
    eff_pr = 1 / (0.5 + np.sqrt(0.25 + ((alpha * eff_th * fhv) / (2 * (1 + bpr) * vtas ** 2))))

    tsfc = vtas / (eff_th * eff_pr * fhv)
...
def tsfc_mattingly_hbpr(tamb, vtas):
    ...
    tsfc = unit.convert_from("lb/lbf/h", (0.4 + 0.45 * mach) * np.sqrt(tamb/t0))
...
def tsfc_mattingly_lbpr(tamb, vtas):
    ...
    tsfc = unit.convert_from("lb/lbf/h", (1.0 + 0.35 * mach) * np.sqrt(tamb/t0))
...
def tsfc_mattingly(t41, opr, fpr, bpr, tamb, vtas):
    ...
    v_core_jet_o_a0 = np.sqrt((2 / (gam - 1)) * (
            tau_lambda - tau_r * (tau_c - 1 + bpr * (tau_f - 1)) - tau_lambda / (tau_r * tau_c)))

    v_fan_jet_o_a0 = np.sqrt((2 / (gam - 1)) * (tau_r * tau_f - 1))

    thrust_ratio = (v_core_jet_o_a0 - mach) / (v_fan_jet_o_a0 - mach)

    thrust_o_air_flow = (a0 / (1 + bpr)) * (v_core_jet_o_a0 - mach + bpr * (v_fan_jet_o_a0 - mach))

    f = (cp * tamb /fhv ) *(tau_lambda - tau_r *tau_c)

    # tsfc = f / ((1 + bpr)*thrust_o_air_flow)

    eta_th = kth * (1 - 1 / (tau_r * tau_c)) + dth

    eta_pr = 2 * mach * (v_core_jet_o_a0 - mach + bpr * (v_fan_jet_o_a0 - mach)) \
...
# -----------------------------------------------------------------------------------------------------------------------
    ...
    for j in range(len(turbofan_data)):
        ...
        fct_fpr = interpolate.interp1d([1, 9, 12, 15,18], [1.5, 1.3, 1.25, 1.2, 1.15], kind="linear")
        fpr = fct_fpr(bpr)
    ...
    turbofan_data1 = turbofan_data[turbofan_data['speed'] > 232].reset_index(drop=True).copy()
    ...
    plt.plot(bpr_list, tsfc_list1, c="blue",)
    ...
    turbofan_data1 = turbofan_data[turbofan_data['speed'] < 243].reset_index(drop=True).copy()
    ...
    plt.plot(power_list, mass_list, c="blue",)
...
# -----------------------------------------------------------------------------------------------------------------------
    ...
    plt.plot(pw_list, psfc_list1, c="blue",)
