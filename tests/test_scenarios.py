...
from noads.scenarios.multiscenario import MultiScenario
...
def test_temporal_scenario(
    ...
    scenario = TemporalScenario(
        "test", models, constant_inputs, delayed_inputs, delay_time,
        interpolated_inputs, interpolated_time, time_integrated_outputs,
...
def test_multi_scenario(
    ...
    scenario.discipline.add_differentiated_inputs(
        ["A1.constant.c", "A2.constant.c", "fixed.control.d"]
    )
    scenario.discipline.add_differentiated_outputs(
        ["mean.cumulative.x", "mean.cumulative.z"]
    )
    scenario.discipline.compile_jit(pre_run=True)
